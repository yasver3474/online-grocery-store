...
echo'<nav class="navbar navbar-inverse navbar-fixed-top" >
  <div class="container">
	...
	die("failed".mysqli_connect_error());
		...
		$sql="select image,item_name,item_id from items where item_id in (select item_id from orders where user_id='$user_id' and order_id in (select DISTINCT order_id from orders))";
...
while($row=mysqli_fetch_array($result)){
		...
		while($row1=mysqli_fetch_array($result1)){
		 ...
		 echo '</div><div class="panel-footer"><form action="../delete_from_cart.php" method="post"><br><input type="text" name="item_id" value="'.$item_id1.'"><button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-shopping-cart"></span>Delete From Cart</button></form></div>
		 </div><br>';
		}
	}
